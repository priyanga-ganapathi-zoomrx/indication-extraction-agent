#!/usr/bin/env python3
"""
Batch Processor for LiteLLM Indication Extraction Agent

This script processes multiple abstracts using the LiteLLM indication extraction agent
...
def extract_indication_from_response(response_content: str) -> Dict[str, Any]:
    """Extract indication and additional fields from agent response.

    Args:
        response_content: JSON string response from the agent
    ...
    try:
        ...
        try:
            ...
            # Fallback: try to find JSON block if wrapped in markdown
            json_match = re.search(r'```json\s*(.*?)\s*```', response_content, re.DOTALL)
            if json_match:
                parsed = json.loads(json_match.group(1))
            else:
                # Last resort: return raw content as indication if short enough
                if len(response_content) < 500:
                     return {
                        'indication': response_content.strip(),
                        'success': True,
                        'selected_source': '',
                        'reasoning': '',
                        'confidence_score': None,
...
def process_single_abstract(abstract: Dict, agent: LiteLLMIndicationAgent,
                           model_name: str, index: int) -> Dict:
    """Process a single abstract and return the result.

    Args:
        abstract: Abstract dictionary
        agent: Initialized LiteLLMIndicationAgent
    ...
    try:
        ...
        extracted_data = extract_indication_from_response(response_content)
        ...
        result_row = {
            ...
            # Note: LiteLLM agent doesn't currently return call count in the run output string
...
def process_abstracts_batch(abstracts: List[Dict], agent: LiteLLMIndicationAgent,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Batch Process Indication Extraction (LiteLLM)')
    ...
    parser.add_argument('--model_name', default='litellm_gemini',
    ...
    print("🏭 Batch Indication Extraction Processor (LiteLLM)")
    print("=" * 80)
