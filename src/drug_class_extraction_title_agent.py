"""Drug Class Extraction from Title Agent.

This module implements an agent that extracts explicit drug classes from abstract titles
using a 3-message prompt structure optimized for reasoning models.

Key features:
- Extracts only explicitly mentioned drug classes (no inference from drug names)
- Applies 36 extraction rules for normalization and validation
- Uses 3-message structure: SYSTEM_PROMPT, RULES_MESSAGE, INPUT_TEMPLATE
- Returns structured JSON output with confidence scores and reasoning
"""
...
# Prompt names
DRUG_CLASS_EXTRACTION_TITLE_PROMPT_NAME = "DRUG_CLASS_EXTRACTION_FROM_TITLE"
DRUG_CLASS_EXTRACTION_RULES_PROMPT_NAME = "DRUG_CLASS_EXTRACTION_FROM_SEARCH_REACT_PATTERN"


class DrugClassExtractionTitleAgent:
    """Drug Class Extraction from Title Agent.

    This agent extracts explicit drug classes from abstract titles using
    a 3-message prompt structure with 36 extraction rules.
    """

    def __init__(
        self,
        agent_name: str = "DrugClassExtractionTitleAgent",
        llm_model: str | None = None,
        temperature: float = None,
        max_tokens: int = None,
        enable_caching: bool = False,
    ):
        """Initialize the Drug Class Extraction Title Agent.
    ...
    def _load_prompt_sections(self) -> tuple[str, str, str]:
        ...
        - SYSTEM_PROMPT and INPUT_TEMPLATE from DRUG_CLASS_EXTRACTION_FROM_TITLE.md
        ...
        prompt_content, prompt_version = get_system_prompt(
            ...
            prompt_name=DRUG_CLASS_EXTRACTION_TITLE_PROMPT_NAME,
        ...
        # Extract INPUT_TEMPLATE section (MESSAGE_2 in title prompt file)
        input_start = prompt_content.find("<!-- MESSAGE_2_START: INPUT_TEMPLATE -->")
        input_end = prompt_content.find("<!-- MESSAGE_2_END: INPUT_TEMPLATE -->")
        if input_start != -1 and input_end != -1:
            input_template = prompt_content[input_start + len("<!-- MESSAGE_2_START: INPUT_TEMPLATE -->"):input_end].strip()
        else:
        ...
        rules_content, rules_version = get_system_prompt(
            langfuse_client=self.langfuse,
            prompt_name=DRUG_CLASS_EXTRACTION_RULES_PROMPT_NAME,
            fallback_to_file=True,
        )
    ...
    def _format_input_message(self, abstract_title: str) -> str:
        """Format the input message by substituting the abstract title.

        Args:
            abstract_title: The abstract title to extract drug classes from

        Returns:
            str: Formatted input message with abstract title substituted
        """
        return self.input_template.replace("{abstract_title}", abstract_title)
    ...
    def invoke(
        ...
        """Invoke the extraction agent to extract drug classes from an abstract title.
        ...
        Returns:
            dict: Extraction result containing drug_classes, source, confidence_score,
                  reasoning, extraction_details, and metadata
        """
        # Handle edge case: empty abstract title
        if not abstract_title or not abstract_title.strip():
            return self._default_extraction_response(
        ...
        tags = [
            ...
            "drug_class_extraction_title",
        ]

        # Format the input message with the abstract title
        input_message = self._format_input_message(abstract_title)
        ...
        try:
            ...
            return self._parse_extraction_response(response, abstract_title, llm_calls=1)
        except Exception as e:
            print(f"✗ Error during extraction LLM call: {e}")
            return self._default_extraction_response(
                abstract_title, f"Extraction failed due to system error: {str(e)}", llm_calls=1
            )
    ...
    def _parse_extraction_response(
        ...
        Returns:
            dict: Parsed extraction result with raw_json_response
        """
        try:
            ...
            if json_match:
                try:
                    ...
                    return self._format_extraction_result(parsed, llm_calls, raw_json_pretty)
            ...
            return self._default_extraction_response(
                abstract_title, f"Unable to parse extraction response: {content[:500]}", llm_calls, content
            )

        except Exception as e:
            print(f"Error parsing extraction response: {e}")
            content = getattr(response, "content", "") if hasattr(response, "content") else ""
            return self._default_extraction_response(abstract_title, f"Parse error: {e}", llm_calls, content)

    def _format_extraction_result(self, parsed: Dict, llm_calls: int, raw_json_pretty: str) -> Dict[str, Any]:
        """Format the parsed extraction result into a consistent structure.
        ...
        Returns:
            dict: Formatted extraction result with raw_json_response
        """
        return {
    ...
    def _default_extraction_response(
        self, abstract_title: str, reason: str, llm_calls: int = 0, raw_content: str = ""
    ) -> Dict[str, Any]:
        """Create a default extraction response for error cases.
        ...
        Returns:
            dict: Default extraction result
        """
