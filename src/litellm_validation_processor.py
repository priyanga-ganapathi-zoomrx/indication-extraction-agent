#!/usr/bin/env python3
"""
LiteLLM Indication Validation Processor

This script validates previously extracted indications from a CSV file
using the LiteLLMValidationAgent (designed for Gemini 3 Pro compatibility).
It reads extraction results and flags potential errors for manual QC review.

This processor uses LiteLLM SDK directly, bypassing LangChain to avoid
Gemini-specific property stripping issues (e.g., thought_signature error).
...
from src.litellm_validation_agent import LiteLLMValidationAgent
...
def pretty_print_json(data: str) -> str:
    """Pretty print JSON string with indentation.
    
    Args:
        data: JSON string or raw text
        
    Returns:
        Pretty-printed JSON string, or original text if not valid JSON
    """
    if not data:
        return ""
...
def validate_single_extraction(
    extraction: Dict,
    validator: LiteLLMValidationAgent,
    index: int,
) -> Dict:
    ...
    Args:
        ...
        validator: Initialized LiteLLMValidationAgent
    ...
    try:
        ...
        result_row['validation_issues'] = json.dumps(validation_result.get('issues_found', []), indent=2)
        ...
        # Add raw LLM JSON response (pretty printed)
        raw_response = validation_result.get('raw_llm_response', '')
        result_row['llm_json_response'] = pretty_print_json(raw_response)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='LiteLLM Indication Validation Processor (Gemini 3 Pro Compatible)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Basic usage with default Gemini 3 Pro
    python -m src.litellm_validation_processor --input_file data/extraction_results.csv
    
    # Specify a different model
    python -m src.litellm_validation_processor --input_file data/extraction_results.csv --llm_model gemini/gemini-2.5-pro
    
    # Process first 10 rows with 5 workers
    python -m src.litellm_validation_processor --input_file data/extraction_results.csv --max_rows 10 --num_workers 5
        """
    )
    ...
    parser.add_argument('--llm_model', default="gemini/gemini-3-pro-preview",
                       help='LLM model name to use for validation calls (default: gemini/gemini-2.5-pro-preview-05-06)')
    ...
    if not args.output_file:
        ...
        # Add litellm suffix to distinguish from langchain processor
        args.output_file = f"data/{base_name}_litellm_validated_{timestamp}.csv"

    print("🔍 LiteLLM Indication Validation Processor (Gemini 3 Pro Compatible)")
    print("=" * 80)
    print(f"Input file: {args.input_file}")
    print(f"Output file: {args.output_file}")
    print(f"Model name (column prefix): {args.model_name}")
    ...
    # Initialize validation agent with LiteLLM
    print("Initializing LiteLLM Validation Agent...")
    validator = LiteLLMValidationAgent(
        agent_name="LiteLLMValidationProcessor",
