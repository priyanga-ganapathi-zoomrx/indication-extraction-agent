"""Drug Class Validation Agent (LangChain Implementation).
...
Uses LangChain Chat API for LLM calls, enabling comparison with the LiteLLM
...
- Optional Anthropic prompt caching support
"""
...
class DrugClassValidationAgentLangChain:
    """Drug Class Validation Agent using LangChain Chat API.
    ...
    Uses LangChain for LLM invocation with optional Langfuse tracing.
    """

    def __init__(
        self,
        agent_name: str = "DrugClassValidationAgentLangChain",
        ...
        print(f"✓ {self.agent_name} initialized with model: {self._llm_model or settings.llm.LLM_MODEL} (LangChain)")
...
Please perform all 3 validation checks (Hallucination Detection, Omission Detection, Rule Compliance) and return your validation result in the specified JSON format.{empty_notice}"""
    ...
    def invoke(
        ...
        tags = [
            ...
            "langchain",  # Tag to identify LangChain implementation
        ]
